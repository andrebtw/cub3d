...
/*   Updated: 2023/12/14 20:47:38 by anrodri2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mlx_create_window(t_cub *cub)
{
	...
	cub->mlx.win = mlx_new_window(\
	cub->mlx.ptr, RES_WIDTH, RES_HEIGHT, WIN_TITLE);
	if (!cub->mlx.win)
	{
...
{
	print_map(cub);
	mlx_put_image_to_window(cub->mlx.ptr, \
	cub->mlx.win, cub->img.img, RES_WIDTH / 4, RES_HEIGHT / 4);
	return (0);
}
